...
inline uint8_t crc8(const uint8_t* data, const size_t size, const Crc8 type = Crc8::SMBUS) {
    ...
    switch(type) {
        case Crc8::SMBUS:
            return fastcrc.smbus(data, size);

        case Crc8::MAXIM:
            return fastcrc.maxim(data, size);
    }
...
inline uint16_t crc16(const uint8_t* data, const size_t size, const Crc16 type = Crc16::MODBUS) {
    ...
    switch(type) {
        case Crc16::CCITT:
            return fastcrc.ccitt(data, size);

        case Crc16::KERMIT:
            return fastcrc.kermit(data, size);

        case Crc16::MODBUS:
            return fastcrc.modbus(data, size);

        case Crc16::XMODEM:
            return fastcrc.xmodem(data, size);

        case Crc16::X25:
            return fastcrc.x25(data, size);
    }
...
inline uint32_t crc32(const uint8_t* data, const size_t size, const Crc32 type = Crc32::CRC32) {
    ...
    switch(type) {
        case Crc32::CRC32:
            return fastcrc.crc32(data, size);

        case Crc32::POSIX:
            return fastcrc.cksum(data, size);
    }
...
inline uint8_t crc8_update(const uint8_t* data, const size_t size, const Crc8 type = Crc8::SMBUS) {
    ...
    if(!has_initialized) {
        ...
        switch(type) {
            case Crc8::SMBUS:
                return fastcrc.smbus_upd(data, size);

            case Crc8::MAXIM:
                return fastcrc.maxim_upd(data, size);
        }
    }
...
inline uint16_t crc16_update(const uint8_t* data, const size_t size, const Crc16 type = Crc16::MODBUS) {
    ...
    if(!has_initialized) {
        ...
        switch(type) {
            case Crc16::CCITT:
                return fastcrc.ccitt_upd(data, size);

            case Crc16::KERMIT:
                return fastcrc.kermit_upd(data, size);

            case Crc16::MODBUS:
                return fastcrc.modbus_upd(data, size);

            case Crc16::XMODEM:
                return fastcrc.xmodem_upd(data, size);

            case Crc16::X25:
                return fastcrc.x25_upd(data, size);
        }
    }
...
inline uint32_t crc32_update(const uint8_t* data, const size_t size, const Crc32 type = Crc32::CRC32) {
    ...
    if(!has_initialized) {
        ...
        switch(type) {
            case Crc32::CRC32:
                return fastcrc.crc32_upd(data, size);

            case Crc32::POSIX:
                return fastcrc.cksum_upd(data, size);
        }
    }
...
inline crc8_t get_crc8_param(const Crc8 type) {
    switch(type) {
        ...
        case Crc8::MAXIM:
            return std::move(CRCPP::CRC::CRC_8_MAXIM());
    }
...
inline crc16_t get_crc16_param(const Crc16 type) {
    switch(type) {
        case Crc16::CCITT:
            return std::move(CRCPP::CRC::CRC_16_CCITTFALSE());

        case Crc16::KERMIT:
            return std::move(CRCPP::CRC::CRC_16_KERMIT());

        case Crc16::MODBUS:
            return std::move(CRCPP::CRC::CRC_16_MODBUS());

        case Crc16::XMODEM:
            return std::move(CRCPP::CRC::CRC_16_XMODEM());

        case Crc16::X25:
            return std::move(CRCPP::CRC::CRC_16_X25());
    }
...
inline crc32_t get_crc32_param(const Crc32 type) {
    switch(type) {
        ...
        case Crc32::POSIX:
            return std::move(CRCPP::CRC::CRC_32_POSIX());
    }
